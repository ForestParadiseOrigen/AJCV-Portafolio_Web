...
const ProyectosModel = ({data}) => {
    return(
        <section key={data.id} className="min-w-92 w-92 pb-5">
            ...
            <section className=" border-y-2 border-rose-600 p-1 flex flex-row flex-nowrap gap-1">
                ...
                <aside className="hover:bg-rose-600 hover:text-white text-md bg-white p-1 text-vertical whitespace-nowrap font-semibold" href="https://boxicons.com/?query=che"><a href={data.link} target="_blank">GITHUB</a></aside>
            </section>

            <footer className="flex flex-row justify-end">
                <img className='h-full bg-repeat-x' src="/src/assets/img/QR/SVG/danger-line.svg"/>
            </footer>
        </section>
    );
}
